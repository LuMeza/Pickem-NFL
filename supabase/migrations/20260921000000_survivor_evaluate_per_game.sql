-- Bug: Survivor solo evaluaba una semana cuando TODOS sus partidos ya tenian
-- resultado ("exit when not v_week_resolved"), asi que en una semana en curso
-- seguian "vivos" jugadores cuyo equipo ya habia perdido. Ahora cada jugador se
-- evalua en cuanto termina SU partido: si su equipo perdio queda eliminado (con
-- la ventana de revivir de la semana siguiente si le quedan vidas) y si gano
-- avanza a la siguiente semana. Quien tiene su partido pendiente queda
-- 'blocked' y no se evalua en semanas posteriores.
...
-- Recalcula ya todos los grupos para corregir el estado actual (semana 2).
do $$
declare g record;
begin
