-- Recordatorio de deadline (frontend, sin cambios de esquema) + notificacion
-- por correo del resultado semanal de Survivor (sigues vivo / eliminado).
--
-- _survivor_recompute ya recorre semana por semana, usuario por usuario,
-- determinando el desenlace (gano, uso una vida, perdio sin vidas, se le
-- rechazo la solicitud). Esta migracion deja una fila en el outbox
-- survivor_notifications en el mismo momento en que el recalculo determina
-- cada desenlace, en vez de comparar estado viejo vs nuevo por separado: como
-- el recalculo es idempotente (misma secuencia de decisiones si se re-corre
-- con los mismos picks/resultados, ver 20260731040002), un "on conflict do
-- nothing" alcanza para no duplicar la notificacion en corridas repetidas.
--
-- Una Edge Function (survivor-notify) programada por cron lee las filas con
-- sent_at is null, envia el correo y marca sent_at — desacoplado del trigger
-- SQL para que una falla de SMTP no interfiera con el recalculo.
create table if not exists public.survivor_notifications (
  id uuid primary key default gen_random_uuid(),
  group_id uuid not null references public.groups (id) on delete cascade,
  user_id uuid not null references public.profiles (id) on delete cascade,
  week_id uuid not null references public.weeks (id) on delete cascade,
  type text not null check (type in ('alive', 'eliminated')),
  reason text not null check (reason in ('won', 'life_used', 'lost_no_lives', 'life_rejected')),
  created_at timestamptz not null default now(),
  sent_at timestamptz,
  error text,
  constraint survivor_notifications_unique unique (group_id, user_id, week_id, type)
);

create index if not exists survivor_notifications_pending
  on public.survivor_notifications (created_at)
  where sent_at is null;

-- Sin policies: la tabla es un outbox interno, solo la Edge Function (service
-- role) y las funciones security definer de Survivor la tocan.
alter table public.survivor_notifications enable row level security;
