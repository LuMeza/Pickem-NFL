export interface WeeklyAccessApprovedEmailParams {
  displayName: string
  weekLabel: string
  pickUrl: string
}

export function buildWeeklyAccessApprovedEmailHtml(params: WeeklyAccessApprovedEmailParams): string {
  return `<!DOCTYPE html>
<html lang="es">
  <head>
    <meta charset="utf-8" />
            ...
            <tr>
              <td style="padding:16px 32px 0 32px; text-align:center; background-color:#141B23;" bgcolor="#141B23">
                <p style="margin:0; font-size:15px; line-height:1.6; color:#8A97A6;">
                  <strong style="color:#F4F6F8;">${params.displayName}</strong>, el administrador aprobó tu acceso a
                  <strong style="color:#C8FF3D;">${params.weekLabel}</strong> de Pickem Semanal. Ya puedes entrar a hacer tus picks.
                </p>
              </td>
            </tr>
            <tr>
