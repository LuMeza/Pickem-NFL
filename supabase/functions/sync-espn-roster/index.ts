...
import { parseEspnRoster, type EspnRosterResponse } from './mapping.ts'
...
Deno.serve(async (req: Request) => {
  ...
  const summary: RosterSyncSummary = { teamsProcessed: 0, playersSynced: 0, errors: [] }
  ...
  for (const team of teams as { id: string; espn_abbreviation: string | null }[]) {
    ...
    try {
      ...
      const players = parseEspnRoster(body)
