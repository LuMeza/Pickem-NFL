...
- Definir el modelo de datos de predicciones semanales y su relación con
  `games`, `group_members` y `module_access`.
...
`module_access`/`group_members`, para que las policies de RLS y las queries de
...
semanas `pretemporada` o `regular`. La tabla acumulada de temporada, una vez que
...
- No hay datos previos de este módulo. Requiere que las migraciones de
  ...
  no de datos).
- Rollback: eliminar la tabla `weekly_picks` y sus policies; no afecta a otras
  capabilities.
